export default {
    ...
    '"-"(hyphen)': '"-"(横线)',
    '"_"(underscore)': '"_"(下划线)',
    "ID extractor separator": "排序ID分隔符",
    "ID extractor separator desc": "按照该分隔符拆分文件名以提取排序用ID。",
    "ID extractor separator placeholder": "例如：，",
    "ID extractor position": "排序ID位置",
    "ID extractor position desc": "从拆分后的片段中选择要写入的那一段。",
    "ID extractor position first": "使用第一个片段",
    "ID extractor position last": "使用最后一个片段",
    "ID extractor field": "写入的属性名称",
    "ID extractor field desc": "将提取到的ID写入该笔记属性。",
    "ID extractor auto": "保存时自动写入排序ID",
    "ID extractor auto desc": "当文件保存时自动执行排序ID提取。",
    "Retrieval": "检索设置",
    ...
    "extract sorting id": "提取排序用ID",
    "id extractor no active file": "没有可以提取排序ID的活动文件。",
    "id extractor empty filename": "去除空白后文件名为空。",
    "id extractor separator missing": "在文件名中没有找到配置的分隔符。",
    "id extractor empty segment": "拆分文件名后没有得到有效的片段。",
    "id extractor missing field": "请先设置要写入排序ID的属性名称。",
    "id extractor success": "排序ID已写入 {{field}}：{{id}}",
    "id extractor failure": "更新排序ID失败。",
